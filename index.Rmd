...
```{r}
graveola <- get_tidy_audio_analysis("6tDxrq4FxEL2q15y37tXT9")
...
graveola %>%
  ...
  geom_raster() +
  scale_fill_viridis_c(guide = "none") +
  labs(title = "Tempogram Dear Mama by Tupac",vx = "Time (s)", y = "Tempo (BPM)") +
  theme_classic()

```

### Tempogram discussion

As visible in the plot the tempo lies around 85 bpm throughout the whole song. The common tempo for hip hop songs is also between 85 and 95 bpm so this was not a suprising result.  
```{r}
knitr::opts_chunk$set(echo = FALSE)
